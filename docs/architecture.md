...
本プロジェクトの Google リソースは、Google Drive の以下のフォルダに集約されている。

```
マイドライブ/
└── 202410_新フォーム/
    ├── VRChatイベントカレンダー           ← prod: Google Spreadsheet（メインデータ）
    ├── VRChatイベントカレンダー2024        ← prod: Google Form（イベント登録フォーム）
    ├── VRCイベント修正URL 一括請求         ← prod: Google Form（修正URL請求フォーム）
    └── _archive_削除予定/                 ← アーカイブ（使用停止中）
        ├── 検証環境                       ← 旧 dev 用スタンドアロン GAS
        └── 検証環境_複数登録               ← 旧 dev 用テスト GAS
```

> **注意**: `_archive_削除予定/` 内のファイルは現在使用していない。削除前に内容を確認すること。
> Drive 上のフォルダ移動はファイル ID に影響しないため、トリガーやコード内の参照には影響しない。
