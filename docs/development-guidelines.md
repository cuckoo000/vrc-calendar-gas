...
> **注意**: dev 環境は現在未配置。再構築する場合は prod のコードをコピーし、上記の環境依存値を dev 用に差し替えること。

### 4.4 既知の問題

| 問題 | 影響 | 対応方針 |
|------|------|--------|
| dev/ が未配置 | ローカルでの開発・テストが行えない | 次回開発作業時に prod からコピーして再構築 |
...
> **前提**: dev 環境がローカルに配置されていること。未配置の場合は先に再構築すること。
